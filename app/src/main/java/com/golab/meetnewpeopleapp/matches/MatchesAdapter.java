...
public class MatchesAdapter extends RecyclerView.Adapter<MatchesViewHolders> {
    ...
    public void onBindViewHolder(@NonNull MatchesViewHolders holder, int position) {
        if(matchesList.get(position).getLastMessage()!=null) {
            ...
            if(!matchesList.get(position).getLastMessage().isReaded() && !matchesList.get(position)
                    .getLastMessage().getCurrentUser())
                    {
                        holder.lastMessage.setTypeface(null, Typeface.BOLD);
                    }
                    else{ holder.lastMessage.setTypeface(null, Typeface.NORMAL); }
        }
